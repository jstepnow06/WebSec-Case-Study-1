...
function searchItems({ q, category, kind }) {
  const conditions = ["items.status != 'removed'"];
  const params = [];
  ...
  if (q) {
    conditions.push(
      "items.title || ' ' || items.description || ' ' || items.location LIKE ?"
    );
    params.push(`%${q}%`);
  }
  ...
  if (kind && kind !== "all") {
    conditions.push("items.kind = ?");
    params.push(kind);
  }

  sql = sql.replace("WHERE items.status != 'removed'", `WHERE ${conditions.join(" AND ")}`);
